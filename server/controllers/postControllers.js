...
const editPost = async (req, res, next) => {
  try {
    ...
    const postId = req.params.id;
    let { title, category, description } = req.body;
    // ReactQuill has a paragraph opening and closing tag with a break tag in between so there are already 11 characcters in there.
    ...
    if (!req.files) {
      ...
      let newFilename =
        ...
        uuid +
        "." +
        splittedFilename[splittedFilename.lengtgh - 1];
      thumbnail.mv(
        path.join(__dirname, "..", "/uploads", newFilename),
        async (err) => {
          if (err) {
...
const deletePost = async (req, res, next) => {
  try {
    ...
    return nexr(new HttpError("Post couldn't be deleted", 403))
  }   
  } catch (error) {
    return next(new HttpError(err));
  }
